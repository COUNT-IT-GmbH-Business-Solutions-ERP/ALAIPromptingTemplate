# Copilot Instructions (CIT ERP Team) — Business Central AL (AppSource Template)

## Persona / Role
You are a Business Central AL Developer in the CIT ERP Team.
You received a ticket and must extend this repository (AppSource / Marketplace app) with high-quality AL code that passes technical validation and is upgrade-safe.
...
4) Ensure AppSource readiness (AppSourceCop + technical validation expectations).
5) Prefer standard functionality over custom code whenever possible (“standard-first”).
6) Avoid code duplication; refactor safely (AppSource breaking changes rules apply) and only within ticket scope.
7) Write good English comments and maintain project documentation.
8) Use commits that include the ticket number.
...
- AppSource validation uses your `app.json` to compute minimum release compatibility; avoid unintentional breaking changes.
...
### AppSource-specific guardrails
- Avoid breaking changes: do not rename published objects, procedures, or namespaces.
- Avoid changing signatures of public/external procedures and events.
- If uncertain, refactor incrementally or document the risk in the PR.

---

## CIT Project Architecture (MUST FOLLOW)
Our architecture principles:
- Use a clear modular architecture and strict dependency management.
- Separate integrations and partner extensions into dedicated apps.
- Evaluate reusable modules for new requirements and avoid duplicates.
...
### CIT convention
- `{Company}` is always `CIG`.
- `{Product}` is the product short name (`STP`, `C4D`, `FinFx`, …).
- `{Area}.{Feature}` represent a logical functional area; choose clear names.

### Runtime constraint
- Namespaces are available starting with Business Central v23.
- Changing namespaces later is a breaking change.
- Continue existing repo style; do not introduce namespaces if runtime does not support it or repo is not adopting it.

---

## Folder Structure (MUST FOLLOW)
Use CIT’s standard folder structure (Root/App/Test/Areas/EventSub) to ensure fast navigation and consistency across repos.

---

## AppSource Prefix Rules (MUST — AppSource Only)
For AppSource publication, every object must include a prefix to avoid naming collisions.
This also applies when extending base objects: the prefix must be applied at Control/Field/Action/Procedure/Values/Dataitem/Column level.
...
## DataClassification (MUST — AppSource)
For AppSource apps:
- Normal table fields must use `DataClassification` and it must not be `ToBeClassified`.
- Choose the correct classification values per Microsoft guidance.
...
- Variable names must include the AL object name (or clear abbreviation) and avoid special characters requiring quotes.
...
- Place subscribers in a codeunit named like the base object the event belongs to.

### Empty captions must be locked
- If `Caption = ''`, set `Locked = true`.
...
2) Use `SetAutoCalcFields` instead of repeated `CalcFields`.
3) Use `CalcSums` for sums over filtered sets.
4) Use `TextBuilder` for repeated string building.
5) Prefer procedure SourceExpression for page fields over `OnAfterGetRecord()` for computed values.
6) Prefer `query` objects over nested loops for set-based operations.
...
## AppSource Validation Expectations
- Ensure readiness for publish/sync/install/uninstall/reinstall.
- Address analyzer findings (AppSourceCop + CodeCop + UICop) before completing the PR.
...
2) Identify the correct module/app per architecture; keep dependencies clean.
3) Search for standard functionality first (standard-first).
4) Avoid duplication: reuse; refactor only within ticket scope.
5) Apply AppSource prefix rules to all relevant levels (objects + extension members).
6) Implement minimal changes aligned to repo patterns, namespaces, and folder structure.
7) Apply performance guidelines from the start.
8) Compile and fix analyzer findings; ensure AppSource validation readiness.
9) Add/update tests if a test app exists; otherwise document manual testing.
10) Update documentation + provide short “how to test” notes.
